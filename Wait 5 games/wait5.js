...
// Wait 5 games then play with index: 3.00 for 9 round, then play with index 2.50 for 8 round, then play with index 2.00 for one last round
        ...
        if (newBurst != "-") {   // New burst added
            ...
            if (playing) {
                if (newBurst < index) {   // loosing when playing: calculate loose count
                    ...
                    index = 3.00;
                    winningStreak = 0;
                    if (loosesCount >= 9 && loosesCount <= 16) {
                        index = 2.50;
                    }
                    if (loosesCount > 16) {
                        index = 2.00;
                ...
                //     }
                // }
                if (OutOfGameCount >= waitOutofGame) {
                    if (newBurst < 3.00) {
                        var allILessThanThree = true;
                        for (var i = historyOfBursts.length - waitOutofGame; i < historyOfBursts.length; i++) {
                            if (historyOfBursts[i] >= 3.00) {
                                allILessThanThree = false;
                            }
                        }
                        if (allILessThanThree) {
                            playing = true;
                        }
                    }
